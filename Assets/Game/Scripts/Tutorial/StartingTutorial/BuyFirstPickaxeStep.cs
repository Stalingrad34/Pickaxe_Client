...
{
  public class BuyFirstPickaxeStep : ITutorialStep
  {
    ...
    public async UniTask Run(TutorialTargetsContainer targetsContainer, CancellationToken token)
    {
      ...
      var button = targetsContainer.Get<AddPickaxeFloorButton>();
      var pickaxes = ServiceProvider.Get<PickaxesService>();

      gui.ShowTutorialText("starting_tutorial_1");
      player.ShowTutorialArrow(button.transform);

      await UniTask.WaitUntil(() => pickaxes.PickaxesNominal.Value > 0, cancellationToken: token).SuppressCancellationThrow();
    }
